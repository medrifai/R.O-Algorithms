...
class KruskalDialog(BaseDialog):
    def setup_dialog(self):
        self.dialog.title("Kruskal - Paramètres")
        tk.Label(self.dialog, text="Nombre de sommets:", bg="#FFE4F2").pack(pady=5)
        self.vertices_entry = tk.Entry(self.dialog)
        self.vertices_entry.pack(pady=5)

        tk.Button(self.dialog, text="Générer", command=self.generate_mst).pack(pady=10)
    ...
    def generate_mst(self):
        try:
            ...
            if num_vertices <= 0:
                raise ValueError("Le nombre de sommets doit être positif")
            create_and_display_graph(num_vertices, "kruskal")
            self.dialog.destroy()
        except ValueError as e:
            messagebox.showerror("Erreur", str(e))
...
class NordOuestDialog(BaseDialog):
    def setup_dialog(self):
        self.dialog.title("Nord Ouest - Paramètres")
        tk.Label(self.dialog, text="Nombre de sources:", bg="#FFE4F2").pack(pady=5)
        self.sources_entry = tk.Entry(self.dialog)
        self.sources_entry.pack(pady=5)
        ...
        tk.Button(self.dialog, text="Calculer", command=self.calculate_transport).pack(pady=10)
    ...
    def calculate_transport(self):
        try:
            ...
            if sources <= 0 or destinations <= 0:
                raise ValueError("Les nombres doivent être positifs")
            create_and_display_graph(0, "nord_ouest", sources=sources, destinations=destinations)
            self.dialog.destroy()
        except ValueError as e:
            messagebox.showerror("Erreur", str(e))

class MoindreCoutDialog(BaseDialog):
    def setup_dialog(self):
        self.dialog.title("Moindre Coût - Paramètres")
        tk.Label(self.dialog, text="Nombre de sources:", bg="#FFE4F2").pack(pady=5)
        self.sources_entry = tk.Entry(self.dialog)
        self.sources_entry.pack(pady=5)
    ...
    def calculate_transport(self):
        try:
            ...
            if sources <= 0 or destinations <= 0:
                raise ValueError("Les nombres doivent être positifs")
            create_and_display_graph(0, "moindre_cout", sources=sources, destinations=destinations)
            self.dialog.destroy()
        except ValueError as e:
            messagebox.showerror("Erreur", str(e))

class SteppingStoneDialog(BaseDialog):
    def setup_dialog(self):
        self.dialog.title("Stepping Stone - Paramètres")
        tk.Label(self.dialog, text="Nombre de sources:", bg="#FFE4F2").pack(pady=5)
        self.sources_entry = tk.Entry(self.dialog)
        self.sources_entry.pack(pady=5)
        ...
        tk.Button(self.dialog, text="Optimiser", command=self.optimize_transport).pack(pady=10)
    ...
    def optimize_transport(self):
        try:
            ...
            if sources <= 0 or destinations <= 0:
                raise ValueError("Les nombres doivent être positifs")
            create_and_display_graph(0, "stepping_stone", sources=sources, destinations=destinations)
            self.dialog.destroy()
        except ValueError as e:
            messagebox.showerror("Erreur", str(e))